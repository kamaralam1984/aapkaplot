...
// The /admin/* layout already gates this route (redirects non-admins). No
...
export default async function AdminEditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  if (process.env.USE_DB !== "1") {
    return (
      <div className="space-y-6">
        <SectionHeader eyebrow="Admin edit" title="DB is off" subtitle="Edits require the live database." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        eyebrow={`Admin edit · ${p.status}`}
        title={p.title}
        subtitle={`Owner: ${p.owner?.name ?? "—"} (${p.owner?.email ?? "no email"}). Saving will move the listing back to PENDING_REVIEW.`}
      />
